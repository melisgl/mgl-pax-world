...
- [15 Streamlets reference][f4d5]
    - [15.1 Opening and closing][bfc52]
...
by [`with-journaling`][6131] or by [`with-bundle`][12a57].
...
- [function] **record-journal**
    ...
    recorded (see [`with-journaling`][6131] and [`with-bundle`][12a57]) or `nil`.
...
- [function] **replay-journal**
    ...
    replayed (see [`with-journaling`][6131] and [`with-bundle`][12a57]) or `nil`.
...
- [macro] **journaled** *(name &key (log-record :record) version args values condition insertable replay-values replay-condition) &body body*
    ...
    the [In-events][186bc] and [Out-events][48ef]. Between generating the two events,
...
- [condition] **journaling-failure** *[serious-condition][af00]*
    ...
    [`journaled`][6267], [`logged`][23c4f], [`with-replay-filter`][0cce], [`sync-journal`][b2ff], and also
...
- [condition] **journal-error** *[error][d162]*
    ...
    Signalled by [`with-journaling`][6131], [`with-bundle`][12a57] and by
...
generated by [`logged`][23c4f] in its raw plist form.
...
[`with-journaling`][6131] and [`with-bundle`][12a57] control replaying and recording
...
The `log-record` argument of `journaled` and [`logged`][23c4f] controls where
...
- [variable] **\*trace-out-name\*** *t*
    ...
    [In-events][186bc]. See [`make-log-decorator`][e33e].
...
kin [`with-bundle`][12a57]). After the per-event processing described below,
...
- [type] **log-event**
    ...
    [`framed`][5d05] is creating frames of log-events, while the [`logged`][23c4f] generates
...
[In-events][186bc] and [Out-events][48ef] but differ in how they are replayed and
the requirements on their [block][06a7]s. The following table names the
type of [`event`][a394] produced (`Event`), how [In-events][186bc] are
...
- [macro] **framed** *(name &key log-record args values condition) &body body*
    ...
    omitted. The related [`logged`][23c4f] creates a single [`leaf-event`][5cd1].
...
- [glossary-term] **invoked**
    ...
    [In-events][186bc] that are always predictable from the code and the
...
The replay process for both [In-events][186bc] and [Out-events][48ef] starts by
...
- [macro] **define-file-bundle-test** *(name &key directory (equivalentp t)) &body body*
    ...
    function's `body` is executed in a [`with-bundle`][12a57] to guarantee
...
[`with-journaling`][6131], [`with-bundle`][12a57], or [`:log-record`][a6ac] without `with-journaling`
is thread-safe.
...
dynamic extent of a [`journaled`][6267] [block][06a7] (see [In-events][186bc] and
[Out-events][48ef]) and also by [`logged`][23c4f]. Apart from being part of the
...
[In-events][186bc] and [Out-events][48ef], the following example is a reminder of
...
- [type] **in-event**
    ...
    [`event-version`][1f5f], and [`event-args`][3335]. See [In-events][186bc] for a more
...
- [type] **leaf-event**
    ...
    Leaf events are triggered by [`logged`][23c4f]. Unlike [`in-event`][1729]s and
...
- [function] **journal-divergent-p** *journal*
    ...
    role in [`with-bundle`][12a57] deciding when a journal is important enough to
...
[`with-bundle`][12a57] automates. Here, we provide a reference for the bundle
classes.
...
- [class] **bundle**
    ...
    replay (see [`with-bundle`][12a57]), the latest journal in the bundle in
...
- [generic-function] **read-event** *streamlet &optional eoj-error-p*
    ...
    [`input-streamlet-p`][b292] or not [`open-streamlet-p`][5da85].
...
- [glossary-term] **readable**
  ...
  [12a57]: #JOURNAL:WITH-BUNDLE%20MGL-PAX:MACRO "JOURNAL:WITH-BUNDLE MGL-PAX:MACRO"
  ...
  [186bc]: #JOURNAL:@IN-EVENTS%20MGL-PAX:SECTION "In-events"
  ...
  [23c4f]: #JOURNAL:LOGGED%20MGL-PAX:MACRO "JOURNAL:LOGGED MGL-PAX:MACRO"
  ...
  [5da85]: #JOURNAL:OPEN-STREAMLET-P%20GENERIC-FUNCTION "JOURNAL:OPEN-STREAMLET-P GENERIC-FUNCTION"
  ...
  [bfc52]: #JOURNAL:@OPENING-AND-CLOSING%20MGL-PAX:SECTION "Opening and closing"
